...
import { getUsers, banUser, unbanUser, approveHostApplication, rejectHostApplication, revokeHostAccess, sendKycReminder } from '../../../../services/adminService';
...
const UsersManagement = () => {
  ...
  const [sendingKycId, setSendingKycId] = useState(null);
  ...
  const handleSendKycReminder = async (user) => {
    if (!user || !user.id) return;
    setSendingKycId(user.id);
    try {
      await sendKycReminder(user.id);
      setKycToast({
        type: 'success',
        message: `KYC verification reminder email sent to ${user.name} (${user.email})!`
      });
      setTimeout(() => setKycToast(null), 5000);
    } catch (err) {
      console.error('Error sending KYC reminder:', err);
      setKycToast({
        type: 'error',
        message: err?.response?.data?.message || err?.message || 'Failed to send KYC reminder email'
      });
      setTimeout(() => setKycToast(null), 5000);
    } finally {
      setSendingKycId(null);
    }
  };
    ...
    <div className="p-6 bg-slate-50 min-h-screen font-inter relative">
            ...
            <div className="w-full flex flex-col pb-24">
              {paginatedUsers.map((user, index) => {
                ...
                const renderKycBadge = () => {
                  ...
                  if (isVerified) {
                    return (
                      <span className="px-2.5 py-[5px] rounded-[20px] text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-200 flex items-center gap-1">
                  ...
                  const isPending = rawKyc === 'PENDING' || rawKyc === 'IN_PROGRESS' || rawKyc === 'SUBMITTED';
                  const isRejected = rawKyc === 'REJECTED' || rawKyc === 'FAILED';
                  
                  return (
                    <div className="flex items-center gap-1.5">
                      <span className={`px-2 py-[4px] rounded-[20px] text-[9px] font-bold uppercase tracking-wider border ${
                        isPending ? 'bg-amber-50 text-amber-700 border-amber-200' :
                        isRejected ? 'bg-rose-50 text-rose-700 border-rose-200' :
                        'bg-slate-100 text-slate-500 border-slate-200'
                      }`}>
                        {isPending ? 'Pending' : isRejected ? 'Rejected' : 'Unverified'}
                      </span>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSendKycReminder(user);
                        }}
                        disabled={sendingKycId === user.id}
                        title="Send KYC verification reminder email"
                        className="p-1 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white transition-all cursor-pointer border border-blue-200 shadow-xs disabled:opacity-50"
                      >
                ...
                >
                  ...
                  <div className={`w-8 h-8 flex-shrink-0 relative ${openMenuId === user.id ? 'z-[60]' : 'z-50'}`}>
                    ...
                    {/* Dropdown Menu */}
                    {openMenuId === user.id && (
                      <div className={`absolute ${isLastItems ? 'bottom-full mb-2' : 'top-full mt-2'} right-0 bg-white rounded-xl border border-slate-100 shadow-2xl shadow-slate-200/60 z-50 p-1.5 w-52 animate-in fade-in zoom-in duration-200`}>
                        <div className="flex flex-col gap-0.5">
                          ...
                          {/* Send KYC Reminder Button */}
                          {user.kycStatus !== 'VERIFIED' && user.kycStatus !== 'APPROVED' && user.verified !== true && (
                            <button
                              onClick={() => {
                                handleSendKycReminder(user);
                                setOpenMenuId(null);
                              ...
                              disabled={sendingKycId === user.id}
                              className="w-full px-3 py-2 rounded-lg flex justify-start items-center gap-3 hover:bg-blue-50 hover:text-blue-700 transition-all group cursor-pointer text-blue-600 disabled:opacity-50"
                            >
                              {sendingKycId === user.id ? (
                                <svg className="w-4.5 h-4.5 animate-spin text-blue-600" fill="none" viewBox="0 0 24 24">
                                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"></path>
                                </svg>
                              ) : (
                              ...
                              <div className="text-xs font-bold">Prompt / Remind KYC</div>
                            </button>
                          )}
      ...
      {selectedUser && (
        <ViewUserModal
          ...
          user={selectedUser}
          onSendKycReminder={handleSendKycReminder}
          sendingKycId={sendingKycId}
        />
      )}

      {/* KYC Reminder Notification Toast */}
      {kycToast && (
        <div className={`fixed bottom-6 right-6 z-[100] px-5 py-3.5 rounded-xl shadow-2xl flex items-center gap-3 text-sm font-semibold border backdrop-blur-sm animate-in slide-in-from-bottom-5 duration-300 ${
          kycToast.type === 'success' ? 'bg-slate-900 text-emerald-400 border-emerald-500/30' : 'bg-rose-950 text-rose-300 border-rose-500/30'
        }`}>
